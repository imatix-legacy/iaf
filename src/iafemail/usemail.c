...
#define APPLICATION_VERSION "2.01"
...
check_new_request (void)
{
    ...
    if (usemail_connect (dbname, dbuser, dbpwd))
      {
        if (debug_mode)
            coputs ("Connected :-)");

        rec_count = usemailqueue_get_all (gmtimestamp_now (), &req);
        if (debug_mode)
        ...
        for (index = 0; index < rec_count &&  control_break == FALSE; index++)
          {
            ...
            feedback = usemaillog_get (&email_log);
            if (feedback == OK)
              {
                ...
                /* Set sender address                                        */
                if (email_log.usfromaddr && *email_log.usfromaddr)
                    smtp.strSenderUserId     = email_log.usfromaddr;
                else
                ...
                if (email_log.usreplyaddr && *email_log.usreplyaddr)
                    smtp.strRetPathUserId    = email_log.usreplyaddr;
                else
                ...
                if (feedback != SMTP_NO_ERROR)

                  {
                    ...
                    feedback = usemaillog_update (&email_log);
                    if (feedback != OK)
                      {
                        sprintf (message, "Error on update email log: %s",
                                           usemail_error_message ());                
                ...
                else
                  {
                    ...
                    email_log.ussentat = gmtimestamp_now ();
                    if (usemaillog_update (&email_log) == OK)
                        usemailqueue_delete (&req [index]);
                    else
                      {
                        sprintf (message, "Error on update email log: %s",
