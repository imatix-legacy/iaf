    ...
    const char *filename)
{
    ...
    /* We check if we have no access to this file in the last 3 seconds      */
#define FILE_STABLE_DELAY  3
    ...
    if (stat (filename, &stat_buf) == 0)
      {
        if (stat_buf.st_size > 0)
          {
            if (stat_buf.st_atime < (cur_time - FILE_STABLE_DELAY))
                feedback = TRUE;
          }
      }

#if defined (__WINDOWS__)
    /* On windows, some copy method preserve ctime and atime !!              */
    if (feedback == FALSE
    ...
    const char *filename)
{
    ...
    rc = fwrite (descr-> data, descr-> size, 1, file_stream);
    fclose (file_stream);
    if (rc != 1)
        return -1;
...
/*  ---------------------------------------------------------------------[<]-
    ...
    Synopsis: Detect if a file content textual or binnaries data.
    Return -1 if error, 0 for binnarie and 1 for textual.

    Get the first kiloBytes of data and check if content binary characters
...
#   endif
    ...
    if ((src_buffer = mem_alloc (SHRT_MAX)) == NULL)
        feedback = -1;                  /*  Insufficient memory for buffer   */
    else
      {
