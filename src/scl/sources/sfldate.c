...
timer_to_gmdate (time_t time_secs)
{
    ...
    if (time_secs == 0)
    ...
    else
      {
        ...
        if (time_struct == NULL)              /*  gmtime is not implemented  */
          {
            adjusted_time = (qbyte) time_secs + local_tz(); /*  Fake gmtime  */
            time_struct = safe_localtime (&adjusted_time);
          }
...
timezone_string (void)
{
    ...
    static char *
        timezone;
    timezone = deletestring((void *)strdupl \
        (env_get_string ("ZONE", "+0000")), "DST", 0);
    return (timezone);
...
*safe_localtime (const time_t *time_secs)
{
    ...
    adjusted_time = (qbyte) *time_secs + local_dst();
...
#if (defined __TURBOC__)                /*  bcc 5.5 is wrong by an hour     */
    adjusted_time += 3600;
#endif
