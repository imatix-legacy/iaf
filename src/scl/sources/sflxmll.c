                 ...
                 Bool  allow_extended)
{
    ...
    buf = init_xml_buffer_from_string (xmlstring);
    if (!buf)
        res = XML_FILEERROR;
...
init_xml_buffer_from_string (const char *xml_string)
{
    ...
    if (res)
      {
        res-> fd = NULL;
        res-> page = mem_strdup(xml_string);
        res-> pageln = strlen(xml_string);
        res-> cur = 0;
        res-> state = BUF_READY;
        res-> cur_line[0] = '\n';
        res-> line_nbr = 0;
            ...
            if (! buf-> page)
              {
                ...
                if (!(buf-> page = (char*) mem_alloc (MAX_BUF_SIZE)))
                  {
                    buf-> state = BUF_UNEXPECTED_ERROR;
    ...
    if (res == XML_NOERROR)
      {
        ...
        if (c == '!')
          {
            /*  it is a comment. We skip it.  syntax is : <!-- blahblah -->  */
            /*  we want 2 dashes '--' as first characters                    */
            buf_get_next(buf, c);
            if (c != '-')
              {
