/*  ----------------------------------------------------------------<Prolog>-
    Name:       usemail.c
    Title:      UltraSource EMail daemon for Windows NT/95/98 service model

    Written:    2000/04/17  iMatix Corporation <info@imatix.com>
    Revised:    2000/04/21

    Synopsis:   Runs the SMTHTTP agent as a NT service.
...
#define APPLICATION_NAME    "usemail"     /* Name of the executable          */
#define APPLICATION_VERSION "1.2"
#define SERVICE_NAME        "usemail"
#define SERVICE_TEXT        "UltraSource Email Daemon"

/*- Global definitions ------------------------------------------------------*/

#define DEPENDENCIES      ""
#define REGISTRY_USEMAIL  "SOFTWARE\\imatix\\usemail"
...
static char *get_dbio_feedback   (int feedback);
...
service_start (int argc, char **argv)
{
    ...
    if (argparm)
      {
        add_to_message_log ("Argument missing - type 'usemail -h' for help");
        return;
      }
    else
    if (!args_ok)
      {
        add_to_message_log ("Invalid arguments - type 'usemail -h' for help");
        return;
      }
        ...
        if (add)
          {
            ...
            feedback = RegSetValueEx (key, "usemail", 0, REG_SZ,
                                     (CONST BYTE *) path, strlen (path) + 1);
            coprintf ("USEmail: service '%s' installed", service_name);
            coputs   ("Restart windows to run service...");
          }
        else
          {
            feedback = RegDeleteValue (key, "usemail");
        ...
        if (streq (xml_item_name (item), "main"))
          {
            ...
            strcpy (trace_file,   xml_get_attr (item, "trace_file",  "usemail.log"));
            strcpy (service_name, xml_get_attr (item, "service-name", SERVICE_NAME));
            strcpy (service_text, xml_get_attr (item, "service-text", SERVICE_TEXT));
          }
        else
...
check_new_request (void)
{
    ...
    memset (&email_log, 0, sizeof (USEMAILLOG));

    if (debug_mode)
        coprintf ("Try connection to %s ODBC data source...", dbname);
    if (emaildb_connect (dbname, dbuser, dbpwd))
      {
        if (debug_mode)
            coputs ("Connected :-)");

        rec_count = emailqueue_get_all (gmtimestamp_now (), &req);
        if (debug_mode)
            coprintf ("Have %ld request in queue", rec_count);
        for (index = 0; index < rec_count &&  control_break == FALSE; index++)
          {
            email_log.usid = req [index].usemaillogid;
            feedback = emaillog_get (&email_log);
            if (feedback == OK)
              {
                memset (&smtp, 0, sizeof (SMTP));
                smtp.strSmtpServer       = smtp_server;
                smtp.strMessageBody      = email_log.usbody;
                smtp.strSubject          = email_log.ussubject;
                smtp.strSenderUserId     = sender;
                smtp.strFullSenderUserId = "";
                smtp.strDestUserIds      = email_log.usrecipients;
                smtp.strFullDestUserIds  = "";
                smtp.strRetPathUserId    = smtp.strSenderUserId;
                smtp.strMsgComment       = "";
                smtp.strMailerName       = "UltraSource EMail Daemon";
                if (user_cc && *user_cc)
                ...
                if (feedback != SMTP_NO_ERROR)

                  {
                    ...
                    emailqueue_delete (&req [index]);

                    email_log.usstatus = 4;
                    email_log.ussentat = gmtimestamp_now ();
                    sprintf (email_log.usmessage, "Send email error: %s",
                             smtp_error_description (feedback));
                    feedback = emaillog_update (&email_log);
                    if (feedback != OK)
                      {
                        sprintf (message, "Error on update email log: %s",
                                           emaildb_error_message ());                
                    ...
                    if (debug_mode)
                        coprintf ("SMTP error %s", email_log.usmessage);
                  }
                else
                  {
                    email_log.usstatus = 2;
                    email_log.ussentat = gmtimestamp_now ();
                    if (emaillog_update (&email_log) == OK)
                        emailqueue_delete (&req [index]);
                    else
                      {
                        sprintf (message, "Error on update email log: %s",
...
/*  ---------------------------------------------------------------------[<]-
    Function: get_dbio_feedback

    Synopsis: Return description of dbio feedback
    ---------------------------------------------------------------------[>]-*/

static char *
get_dbio_feedback (int feedback)
{
    static char *feedback_msg [] = {
    "OK",
    "Hardware error",
    "ILLEGAL_OPERATION",
    "ILLEGAL_MODE",
