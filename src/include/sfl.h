...
#if (defined (__EMX__) && defined (__i386__) && !defined (__RSXNT__))
#   undef __OS2__
#   define __OS2__
#endif
...
#           include <sys/types.h>       /*  Required before <direct.h>       */
#       endif
#       include <direct.h>   
#       if (!defined (CONSOLE))
#          include <windows.h>  
...
#endif
#if (!defined (__MINGW32__))            /*    Throws parse error - RJ        */
...
#      define _fileno fileno
       extern long get_osfhandle(int);   /*  Missing from <io.h>             */
...
#   endif
    /*  Support for lcc-win32, gnu derivatives and borland                   */
#   if (defined __LCC__ || defined __MINGW32__ || defined __TURBOC__ \
    ||  defined __RSXNT__)
#       if (!defined CONSOLE)
#          include <mmsystem.h>
#       endif
...
#       endif
#       if (defined __RSXNT__)          /*  There's always _one_ exception!  */
#          define environ _environ
#       endif
...
#   endif
    ...
    || defined (__UTYPE_NETBSD) || defined __DJGPP__)
#   undef  TIMEZONE
 ...
 *  DOES_UNICODE         Has basic unicode support (wcscat, etc; C99, Unix98)
 *  DOES_UNICODE_PRINTF  Has unicode printf() like functions
 */
...
#   undef  DOES_SOCKETS
                                        ...
                                        /*  Assume no sockets available      */
#if (defined CONSOLE || defined __DJGPP__)
#   undef DOES_SOCKETS
#endif
...
#else
#   undef  DOES_UID
           /*  uid_t and gid_t already defined  */
...
#if (defined (CONSOLE) && !defined (__GNUC__))
        typedef void *pid_t;            /*  Process id type                  */
#endif

#if (defined (__WINDOWS__) || defined (__OS2__) || defined (__UTYPE_SCO) \
   || defined (__UTYPE_LINUX))
...
#if (defined (__UTYPE_LINUX) || defined (__UTYPE_FREEBSD) \
  || defined (__NETBSD) || (defined __WINDOWS__ && defined __LCC__))
#define DOES_SETENV
#else
#undef  DOES_SETENV
#endif
...
#if (defined (__UTYPE_LINUX) || defined (__WINDOWS__))
#define DOES_UNICODE
#    if (defined (__UTYPE_LINUX))
#        include <wchar.h>
#        define SFL_UNICODE_TYPE   wchar_t
         /* Linux does not yet have Unicode printf() functions               */
#        undef  DOES_UNICODE_PRINTF
#    else
#        define DOES_UNICODE_PRINTF
#    endif
#endif

#if (defined (CONSOLE))
#undef __WINDOWS__                      /*  That's the last we need of them  */
#undef WIN32
#endif
...
/*  ----------------------------------------------------------------<Prolog>-
    Name:       sflunic.h
    Title:      Unicode support functions
    Package:    Standard Function Library (SFL)

    Written:    2000/12/29  iMatix SFL project team <sfl@imatix.com>
    Revised:    2001/10/10

    Synopsis:   Provides functions to read and write text files with unicode
                support and unicode string function.

    Copyright:  Copyright (c) 1996-2001 iMatix Corporation
    License:    This is free software; you can redistribute it and/or modify
                it under the terms of the SFL License Agreement as provided
                in the file LICENSE.TXT.  This software is distributed in
                the hope that it will be useful, but without any warranty.
...
/*  ----------------------------------------------------------------<Prolog>-
    Name:       sfldes.h
    Title:      DES Encryption and decryption functions
    Package:    Standard Function Library (SFL)
...
/*  ----------------------------------------------------------------<Prolog>-
    ...
    Synopsis:   The directory access functions provide a portable interface
                ...
                OS/2 port was done by Ewen McNeill <ewen@naos.co.nz>.
                Changes for Win32 by Will Menninger <willus@netcom.com>.
                DOS box compatibility was done by Rob Judd <judd@alphalink.com.au>
                including support for lcc-win32, bcc 5.5, lcc 4.1, DJGPP, mingw32,
                cygnus, rsxnt/djgpp, DRDOS LFN and the -DCONSOLE switch.
...
#if (!defined DOES_UID && !defined __DJGPP__ && !defined __MINGW32__ \
  && !defined __TURBOC__ && !defined __RSXNT__)
...
#elif (defined (CONSOLE))               /*  Plain ol' DOS, or DOS box  - RJ  */
    long    _dir_handle;                /*    a directory handle             */
    struct  _finddata_t _dir_entry;     /*    and a file desc. structure     */
...
/*  ----------------------------------------------------------------<Prolog>-
    Name:       sflusymb.h
    Title:      Unicode Symbol-table functions
    Package:    Standard Function Library (SFL)
...
/*  ----------------------------------------------------------------<Prolog>-
    Name:       sfluhttp.c
    Title:      HTTP and CGI Support functions in unicode
    Package:    Standard Function Library (SFL)
...
/*  ----------------------------------------------------------------<Prolog>-
    Name:       sflidea.h
    Title:      IDEA Encryption and decryption functions
    Package:    Standard Function Library (SFL)
...
# define mem_free(p)        { if ((p)) mem_free_ ((p), NULL, 0); }
...
#   if (!defined CONSOLE || defined __UTYPE_BEOS)
...
#   if (!defined (__DJGPP__) && !defined (CONSOLE))
...
/*--------------------------------------------------------------------------
 *  Fix for rsxnt's broken or missing endian conversion functions
 */
...
#if (defined FAKE_SOCKETS || defined __UTYPE_BEOS || defined __RSXNT__)
...
typedef void (CONSOLE_UFCT) (const UCODE *);
...
/*  ----------------------------------------------------------------<Prolog>-
    Name:       sflrc4.h
    Title:      RC4 Encryption and decryption functions
    Package:    Standard Function Library (SFL)

    Written:    98/01/06  iMatix SFL project team <sfl@imatix.com>
    Revised:    97/01/06
...
/*  ----------------------------------------------------------------<Prolog>-
    ...
    Revised:    1999/10/17
                2000/06/14  Support for -DCONSOLE  - RJ
...
#if (defined __UNIX__ || defined __OS2__ || defined WIN32 || defined CONSOLE)
...
/*  ----------------------------------------------------------------<Prolog>-
    Name:       sflsha.h
    Title:      SHA hashing functions
    Package:    Standard Function Library (SFL)

    Written:    98/01/31  iMatix SFL project team <sfl@imatix.com>
    Revised:    98/01/31
...
/*  ----------------------------------------------------------------<Prolog>-
    ...
    Revised:    2000/06/03  Jonathan Schultz <jonathan@imatix.com>
...
/*  ----------------------------------------------------------------<Prolog>-
    Name:       sfluxml.h
    Title:      Unicode version of XML access functions
    Package:    Standard Function Library (SFL)

    Written:    2001/01/09  iMatix SFL project team <sfl@imatix.com>
    Revised:    2001/01/10
...
/*  ----------------------------------------------------------------<Prolog>-
    ...
    Written:    2001/01/10  iMatix SFL project team <sfl@imatix.com>
    Revised:    2001/01/10
